...
@register('neg_precision')
def precision(outputs):
    target = 1-np.array(outputs['target'])
    pred = 1-np.array(outputs['pred'])
    return {
        'neg_precision': metrics.precision_score (target, pred).item (),
    }
...
def precision(outputs):
    ...
    pred = 1 - np.array (outputs['pred'])
    return {
        'neg_recall': metrics.recall_score (target, pred).item (),
    }
