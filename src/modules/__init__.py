...
def get_weight(shape, gain=np.sqrt(2), weight_norm=True, fan_in=None, name='weight'):
    ...
    w = tf.get_variable(name, shape=shape, initializer=tf.initializers.random_normal(0, std),regularizer=tf.contrib.layers.l2_regularizer(0.01),
                        dtype=tf.float32)
    if weight_norm:
...
def conv2d(x, filters, kernel_size, activation=None, name='conv1d'):
    with tf.variable_scope(name):
        gain = np.sqrt(2) if activation is tf.nn.relu else 1
        w = get_weight([kernel_size,kernel_size, x.shape[-1].value, filters], gain=gain)
        out = tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding='SAME')
        out = apply_bias(out)
        if activation:
            out = activation(out)
